...
const submit = async () => {
    if (!form.username || !form.password) {
      return setErreur('Nom et mot de passe obligatoires');
    }
    setLoading(true);
    setErreur('');
    try {
      const res = await axios.post(
        'http://127.0.0.1:8000/api/auth/inscription/',
        form
      );
      if (res.data.token) {
      ...
      if (err.response?.data?.username) {
        ...
        const erreurs = Object.values(err.response.data).flat();
        setErreur(erreurs[0] || "Erreur lors de l'inscription.");
      } else {
